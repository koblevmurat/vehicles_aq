...
public class ResponseRepository implements IResponseRepository {
    ...
    @Value("${1c.datasource.userName1c}")
    private String userName1c;

    @Value("${1c.datasource.password1c}")
    private String password1c;

    //@Value("${Url1c}")
    @Value("${1c.datasource.url}")
...
//        HttpResponse<String> response = client.send(request, HttpResponse.BodyHandlers.ofString());
    ...
    public int UpdateDocument(DocStatus mapDocStatus) throws URISyntaxException, IOException, InterruptedException {
